...
MongoDB stores data in collections. Each collection contains documents. Documents for the same collection usually represent the same kind of thing.

## Real-World Analogy

A collection is like a drawer in a filing cabinet. Each document is one file inside that drawer.
...
```js
// This example shows a collection as a list of related documents.
const lessonsCollection = [
  { title: "HTML Basics", level: "beginner" },
  { title: "CSS Grid", level: "intermediate" },
];

console.log(lessonsCollection);
```
...
Next chapter: **MongoDB Shell Basics**.
